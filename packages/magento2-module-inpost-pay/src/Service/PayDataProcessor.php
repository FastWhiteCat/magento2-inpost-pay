...
class PayDataProcessor
{
    ...
    public function process(int $quoteId, array $params, RequestInterface $request): array
    {
        if ($basketId = $this->tryBindExistingBasket($quoteId)) {
            return ['basket_id' => $basketId];
        }

        $browser = $this->base64serializer->unserialize($params['browser']);
        $browserData = [];
        if (is_array($browser)) {
            $browserData = $this->prepareBrowserData($browser, $request);
        }
